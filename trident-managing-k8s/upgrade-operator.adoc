...
[role="lead"]
Puedes actualizar Astra Trident utilizando el operador Trident, ya sea de forma manual o mediante Helm. Puedes actualizar de una instalación del operador Trident a otra instalación del operador Trident o actualizar de una instalación de  `tridentctl` a una versión del operador Trident. Revisa link:upgrade-trident.html#step-3-select-an-upgrade-method["Seleccione un método de actualización"] antes de actualizar una instalación del operador Trident.
...
NOTE: Para actualizar desde Astra Trident que se instaló mediante el operador de ámbito de espacio de nombres (versiones 20.07 a 20.10), usa las instrucciones de actualización para la versión de Trident que tienes instalada.
...
.Pasos
. Si link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-astra-trident-using-helm["Instalar Astra Trident mediante Helm"], puedes usar  `helm upgrade trident netapp-trident/trident-operator --version 23.10.0` para actualizar en un solo paso. Si no agregaste el repositorio de Helm o no puedes usarlo para actualizar:
+
...
NOTE: Antes de cambiar de método de instalación, revisa link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["Moverse entre los métodos de instalación"]
