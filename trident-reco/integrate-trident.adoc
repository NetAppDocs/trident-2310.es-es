...
| `ontap-nas` | Sí | Sí | Yespie de página:5[] | Sí | Nota de pie de página:1[] | Sí | Nota de pie de página:1[] 


| `ontap-nas-economy` | Nota de pie de página:3[] | Nota de pie de página:3[] | Yespie de página:5[] | Sí | Nota de pie de página:3[] | Sí | Nota de pie de página:3[] 
...
| `solidfire-san` | Sí | Sí | Nota de pie de página:2[] | Sí | Sí | Sí | Nota de pie de página:1[] 
|===
[verse]
Pie de página:
Nota:1[]: No gestionado por Astra Trident
Yesfootnote:2[]: Compatible con volúmenes de bloques sin configurar
...
Astra Trident utiliza el controlador  `azure-netapp-files` para gestionar el servicio link:https://azure.microsoft.com/en-us/products/netapp/["Azure NetApp Files"^].
...
| `openshift_metrics_storage_volume_name` | El nombre,
ej `pv_ose_metrics`, Del PV que se va a crear. 
